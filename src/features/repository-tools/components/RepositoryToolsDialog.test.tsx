...
function createDialogProps(overrides: Partial<RepositoryToolsDialogProps> = {}): RepositoryToolsDialogProps {
  return {
    tab: 'configuration',
    repository: {
      id: 'repository-id',
...
describe('RepositoryToolsDialog publishing account', () => {
  ...
  it('keeps the dialog close action available while a repository operation is busy', () => {
    const html = renderToStaticMarkup(<RepositoryToolsDialog {...createDialogProps({ loading: true })} />)
    const closeButton = html.match(/<button[^>]+aria-label="关闭仓库工具"[^>]*>/)?.[0]
