...
分别管理本地、远端和已归档分支，并随时确认当前检出位置。分支总览左栏固定显示本地分支、右栏固定显示远程分支；带斜杠的分支和标签名称会按目录优先的逐层字母顺序排列。你可以创建和切换分支、合并工作线、拣选或撤销修订、比较分支，以及创建和管理标签。引导式冲突会话支持解决、重新开始或中止合并、拣选和撤销操作。
